...
class CDQAgent:
    def __init__(self, obs_dim, act_dim, max_action, args, device):
        ...
        self.critic1_target = Critic(obs_dim, act_dim, args.hidden_size).to(device)
        self.critic2_target = Critic(obs_dim, act_dim, args.hidden_size).to(device)
    ...
    def update(self, replay_buffer, batch_size):
        ...
        max_action = torch.as_tensor(self.max_action_np, dtype=torch.float32, device=self.device)

        with torch.no_grad():
            noise = torch.randn_like(action) * self.policy_noise * max_action
            noise = torch.clamp(noise, -self.noise_clip * max_action, self.noise_clip * max_action)
        ...
        return critic1_loss.item(), critic2_loss.item(), actor_loss_value

    def save(self, out_dir, env_name, suffix):
        os.makedirs(out_dir, exist_ok=True)
        actor_path = os.path.join(out_dir, f"ddpg_cdq_actor_{env_name}_{suffix}.pth")
        critic_path = os.path.join(out_dir, f"ddpg_cdq_critics_{env_name}_{suffix}.pth")
...
def train(args):
    ...
    agent = CDQAgent(obs_dim, act_dim, max_action, args, device)
    ...
    progress = trange(1, args.max_steps + 1, desc="DDPG-CDQ HalfCheetah")
    for step in progress:
        ...
        if replay_buffer.size >= args.batch_size and step > args.start_steps:
            critic1_loss, critic2_loss, actor_loss = agent.update(replay_buffer, args.batch_size)
            log_dict = {
...
def parse_args():
    parser = argparse.ArgumentParser(description="DDPG with Clipped Double Q for HalfCheetah-v5")
    ...
    parser.add_argument("--project", type=str, default="ddpg-cdq-halfcheetah")
    parser.add_argument("--run-name", type=str, default="ddpg_cdq_cheetah_seed42")
