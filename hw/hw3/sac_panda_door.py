# Spring 2026, 535510 Reinforcement Learning
# HW3: SAC on Robosuite Panda-Door-Opening
# Instructor: Ping-Chun Hsieh (National Yang Ming Chiao Tung University)

import os
import random
from typing import Dict, Tuple

import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
import torch.optim as optim
from torch.distributions import Normal
import argparse
import wandb
from tqdm import tqdm

import robosuite as suite
from robosuite.wrappers import GymWrapper


# ----------------------------------------------------------------------------
# Network utilities
# ----------------------------------------------------------------------------

def init_layer_uniform(layer: nn.Linear, init_w: float = 3e-3) -> nn.Linear:
    """Init the output-layer weights and biases uniformly in [-init_w, init_w]."""
    layer.weight.data.uniform_(-init_w, init_w)
    layer.bias.data.uniform_(-init_w, init_w)
    return layer


class Actor(nn.Module):
    """Gaussian policy with tanh squashing for SAC.

    The network outputs (mu, log_std). Actions are obtained by reparameterized
    sampling z ~ N(mu, sigma) and a tanh squash to (-1, 1).
    """

    def __init__(
        self,
        in_dim: int,
        out_dim: int,
        hidden_dim: int = 256,
        log_std_min: float = -20,
        log_std_max: float = 2,
    ):
        super(Actor, self).__init__()

        self.log_std_min = log_std_min
        self.log_std_max = log_std_max

        self.hidden1 = nn.Linear(in_dim, hidden_dim)
        self.hidden2 = nn.Linear(hidden_dim, hidden_dim)

        # log-std head and mean head share the trunk
        self.log_std_layer = init_layer_uniform(nn.Linear(hidden_dim, out_dim))
        self.mu_layer = init_layer_uniform(nn.Linear(hidden_dim, out_dim))

    def forward(self, state: torch.Tensor) -> torch.Tensor:
        x = F.relu(self.hidden1(state))
        x = F.relu(self.hidden2(x))

        # Mean is bounded to (-1, 1) via tanh for numerical stability.
        mu = self.mu_layer(x).tanh()

        # Clamp log-std into a safe range to avoid degenerate distributions.
        log_std = self.log_std_layer(x).tanh()
        log_std = self.log_std_min + 0.5 * (self.log_std_max - self.log_std_min) * (log_std + 1)
        std = torch.exp(log_std)

        # Reparameterization trick (allows gradient flow through the sample).
        dist = Normal(mu, std)
        z = dist.rsample()

        # Tanh squash + change-of-variable correction (Appendix C of SAC paper).
        action = z.tanh()
        log_prob = dist.log_prob(z) - torch.log(1 - action.pow(2) + 1e-7)
        log_prob = log_prob.sum(-1, keepdim=True)

        return action, log_prob

    def get_deterministic_action(self, state: torch.Tensor) -> torch.Tensor:
        """Deterministic action for evaluation (no sampling noise)."""
        x = F.relu(self.hidden1(state))
        x = F.relu(self.hidden2(x))
        mu = self.mu_layer(x).tanh()
        return mu.tanh()


class CriticQ(nn.Module):
    """Soft Q network Q(s, a)."""

    def __init__(self, in_dim: int, hidden_dim: int = 256):
        super(CriticQ, self).__init__()
        self.hidden1 = nn.Linear(in_dim, hidden_dim)
        self.hidden2 = nn.Linear(hidden_dim, hidden_dim)
        self.out = init_layer_uniform(nn.Linear(hidden_dim, 1))

    def forward(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        x = torch.cat((state, action), dim=-1)
        x = F.relu(self.hidden1(x))
        x = F.relu(self.hidden2(x))
        return self.out(x)


class CriticV(nn.Module):
    """Soft value network V(s)."""

    def __init__(self, in_dim: int, hidden_dim: int = 256):
        super(CriticV, self).__init__()
        self.hidden1 = nn.Linear(in_dim, hidden_dim)
        self.hidden2 = nn.Linear(hidden_dim, hidden_dim)
        self.out = init_layer_uniform(nn.Linear(hidden_dim, 1))

    def forward(self, state: torch.Tensor) -> torch.Tensor:
        x = F.relu(self.hidden1(state))
        x = F.relu(self.hidden2(x))
        return self.out(x)


# ----------------------------------------------------------------------------
# Replay buffer
# ----------------------------------------------------------------------------

class ReplayBuffer:
    """Plain numpy replay buffer."""

    def __init__(self, obs_dim: int, act_dim: int, size: int, batch_size: int = 32):
        self.obs_buf = np.zeros([size, obs_dim], dtype=np.float32)
        self.next_obs_buf = np.zeros([size, obs_dim], dtype=np.float32)
        self.acts_buf = np.zeros([size, act_dim], dtype=np.float32)
        self.rews_buf = np.zeros([size], dtype=np.float32)
        self.done_buf = np.zeros([size], dtype=np.float32)
        self.max_size, self.batch_size = size, batch_size
        self.ptr, self.size = 0, 0

    def store(self, obs, act, rew, next_obs, done):
        self.obs_buf[self.ptr] = obs
        self.next_obs_buf[self.ptr] = next_obs
        self.acts_buf[self.ptr] = act
        self.rews_buf[self.ptr] = rew
        self.done_buf[self.ptr] = done
        self.ptr = (self.ptr + 1) % self.max_size
        self.size = min(self.size + 1, self.max_size)

    def sample_batch(self) -> Dict[str, np.ndarray]:
        idxs = np.random.choice(self.size, size=self.batch_size, replace=False)
        return dict(
            obs=self.obs_buf[idxs],
            next_obs=self.next_obs_buf[idxs],
            acts=self.acts_buf[idxs],
            rews=self.rews_buf[idxs],
            done=self.done_buf[idxs],
        )

    def __len__(self) -> int:
        return self.size


# ----------------------------------------------------------------------------
# SAC agent
# ----------------------------------------------------------------------------

class SACAgent:
    """Soft Actor-Critic agent for the Robosuite Panda-Door-Opening task."""

    def __init__(self, env, args=None):
        obs_dim = env.observation_space.shape[0]
        action_dim = env.action_space.shape[0]

        self.env = env
        self.memory_size = args.memory_size
        self.batch_size = args.batch_size
        self.memory = ReplayBuffer(obs_dim, action_dim, self.memory_size, self.batch_size)
        self.gamma = args.discount_factor
        self.tau = args.tau
        self.lr = args.lr
        self.initial_random_steps = args.initial_random_steps
        self.policy_update_freq = args.policy_update_freq
        self.seed = args.seed
        self.num_steps = int(args.num_steps)
        self.eval_interval = args.eval_interval
        self.eval_episodes = args.eval_episodes
        self.checkpoint_dir = args.checkpoint_dir
        self.hidden_dim = args.hidden_dim

        if torch.cuda.is_available():
            self.device = torch.device("cuda")
        elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
            self.device = torch.device("mps")
        else:
            self.device = torch.device("cpu")
        print(f"[Device] {self.device}")

        # ---- Auto-tuned temperature (SAC v2 dual problem) ----
        # Target entropy heuristic: H_0 = -dim(A).
        self.target_entropy = -np.prod((action_dim,)).item()
        self.log_alpha = torch.zeros(1, requires_grad=True, device=self.device)
        self.alpha_optimizer = optim.Adam([self.log_alpha], lr=self.lr)

        # ---- Networks ----
        self.actor = Actor(obs_dim, action_dim, self.hidden_dim).to(self.device)
        self.vf = CriticV(obs_dim, self.hidden_dim).to(self.device)
        self.vf_target = CriticV(obs_dim, self.hidden_dim).to(self.device)
        self.vf_target.load_state_dict(self.vf.state_dict())

        self.qf_1 = CriticQ(obs_dim + action_dim, self.hidden_dim).to(self.device)
        self.qf_2 = CriticQ(obs_dim + action_dim, self.hidden_dim).to(self.device)

        # ---- Optimizers ----
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=self.lr)
        self.vf_optimizer = optim.Adam(self.vf.parameters(), lr=self.lr)
        self.qf_1_optimizer = optim.Adam(self.qf_1.parameters(), lr=self.lr)
        self.qf_2_optimizer = optim.Adam(self.qf_2.parameters(), lr=self.lr)

        self.transition = list()
        self.total_step = 0
        self.is_test = False

    # ------------------------------------------------------------------
    # Environment helpers (robosuite GymWrapper returns may differ across
    # versions, so we keep this defensive).
    # ------------------------------------------------------------------

    def _reset_env(self) -> np.ndarray:
        result = self.env.reset()
        if isinstance(result, tuple):
            return result[0]
        return result

    def _env_step(self, action):
        out = self.env.step(action)
        if len(out) == 5:
            next_state, reward, terminated, truncated, info = out
            done = bool(terminated) or bool(truncated)
        else:  # gym <0.26 style: (obs, reward, done, info)
            next_state, reward, done, info = out
        return next_state, float(reward), bool(done), info

    # ------------------------------------------------------------------
    # Acting
    # ------------------------------------------------------------------

    def select_action(self, state: np.ndarray) -> np.ndarray:
        if self.total_step < self.initial_random_steps and not self.is_test:
            selected_action = self.env.action_space.sample()
        else:
            with torch.no_grad():
                selected_action = (
                    self.actor(torch.FloatTensor(state).to(self.device))[0]
                    .cpu()
                    .numpy()
                )

        self.transition = [state, selected_action]
        return selected_action

    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool]:
        next_state, reward, done, _ = self._env_step(action)

        if not self.is_test:
            self.transition += [reward, next_state, done]
            self.memory.store(*self.transition)

        return next_state, reward, done

    # ------------------------------------------------------------------
    # Learning
    # ------------------------------------------------------------------

    def update_model(self) -> Tuple[torch.Tensor, ...]:
        """One gradient step on Q-, V-, actor- and alpha-networks."""
        device = self.device

        samples = self.memory.sample_batch()
        state = torch.FloatTensor(samples["obs"]).to(device)
        next_state = torch.FloatTensor(samples["next_obs"]).to(device)
        action = torch.FloatTensor(samples["acts"]).to(device)
        reward = torch.FloatTensor(samples["rews"].reshape(-1, 1)).to(device)
        done = torch.FloatTensor(samples["done"].reshape(-1, 1)).to(device)
        new_action, log_prob = self.actor(state)

        # ---- train alpha (auto-tune temperature) ----
        alpha_loss = (-self.log_alpha.exp() * (log_prob + self.target_entropy).detach()).mean()
        self.alpha_optimizer.zero_grad()
        alpha_loss.backward()
        self.alpha_optimizer.step()
        alpha = self.log_alpha.exp()

        # ---- q function loss ----
        # Bellman target uses the target V-network (no gradient through it).
        mask = 1 - done
        ########## Your Code (~10 lines)##########
        with torch.no_grad():
            v_target_next = self.vf_target(next_state)
            q_target = reward + self.gamma * v_target_next * mask

        qf_1_pred = self.qf_1(state, action)
        qf_2_pred = self.qf_2(state, action)

        qf_1_loss = F.mse_loss(qf_1_pred, q_target)
        qf_2_loss = F.mse_loss(qf_2_pred, q_target)
        ########## End of Your Code ##########

        # ---- v function loss ----
        # V_hat(s) = min Q(s, a~pi) - alpha * log pi(a|s)   (clipped double-Q)
        ########## Your Code (~5 lines)##########
        q_min_new = torch.min(
            self.qf_1(state, new_action),
            self.qf_2(state, new_action),
        )
        v_target_value = q_min_new - alpha * log_prob
        vf_loss = F.mse_loss(self.vf(state), v_target_value.detach())
        ########## End of Your Code ##########

        if self.total_step % self.policy_update_freq == 0:
            # ---- actor loss ----
            # L_pi = E[ alpha * log pi(a|s) - min Q(s, a~pi) ]
            ########## Your Code (<5 lines)##########
            q_min_pi = torch.min(
                self.qf_1(state, new_action),
                self.qf_2(state, new_action),
            )
            actor_loss = (alpha.detach() * log_prob - q_min_pi).mean()
            ########## End of Your Code ##########

            self.actor_optimizer.zero_grad()
            actor_loss.backward()
            self.actor_optimizer.step()

            self._target_soft_update()
        else:
            actor_loss = torch.zeros(())

        # ---- Q updates ----
        self.qf_1_optimizer.zero_grad()
        qf_1_loss.backward()
        self.qf_1_optimizer.step()

        self.qf_2_optimizer.zero_grad()
        qf_2_loss.backward()
        self.qf_2_optimizer.step()

        qf_loss = qf_1_loss + qf_2_loss

        # ---- V update ----
        self.vf_optimizer.zero_grad()
        vf_loss.backward()
        self.vf_optimizer.step()

        return actor_loss.data, qf_loss.data, vf_loss.data, alpha_loss.data

    # ------------------------------------------------------------------
    # Evaluation / checkpointing
    # ------------------------------------------------------------------

    def evaluate(self, num_episodes: int = 10) -> Tuple[float, float]:
        """Deterministic evaluation using tanh(mu)."""
        self.actor.eval()
        eval_scores = []

        for _ in range(num_episodes):
            state = self._reset_env()
            done = False
            ep_score = 0.0

            while not done:
                with torch.no_grad():
                    state_tensor = torch.FloatTensor(state).to(self.device)
                    action = self.actor.get_deterministic_action(state_tensor).cpu().numpy()
                next_state, reward, done, _ = self._env_step(action)
                ep_score += reward
                state = next_state

            eval_scores.append(ep_score)

        self.actor.train()
        mean_score = float(np.mean(eval_scores))
        std_score = float(np.std(eval_scores))
        print(f"[Eval episodes] {eval_scores}")
        return mean_score, std_score

    def save_checkpoint(self, checkpoint_dir: str, step: int):
        os.makedirs(checkpoint_dir, exist_ok=True)
        path = os.path.join(checkpoint_dir, f"checkpoint_step{step}.pt")
        torch.save(
            {
                "step": step,
                "actor": self.actor.state_dict(),
                "vf": self.vf.state_dict(),
                "vf_target": self.vf_target.state_dict(),
                "qf_1": self.qf_1.state_dict(),
                "qf_2": self.qf_2.state_dict(),
                "log_alpha": self.log_alpha.detach().cpu(),
                "actor_optimizer": self.actor_optimizer.state_dict(),
                "vf_optimizer": self.vf_optimizer.state_dict(),
                "qf_1_optimizer": self.qf_1_optimizer.state_dict(),
                "qf_2_optimizer": self.qf_2_optimizer.state_dict(),
                "alpha_optimizer": self.alpha_optimizer.state_dict(),
            },
            path,
        )
        print(f"\n[Checkpoint] Saved to {path}")

    # ------------------------------------------------------------------
    # Training loop
    # ------------------------------------------------------------------

    def train(self):
        self.is_test = False

        state = self._reset_env()
        score = 0.0
        ep = 0

        for self.total_step in tqdm(range(1, self.num_steps + 1)):
            action = self.select_action(state)
            next_state, reward, done = self.step(action)
            state = next_state
            score += reward

            actor_loss = qf_loss = vf_loss = alpha_loss = 0

            if done:
                state = self._reset_env()
                ep += 1
                print(f"\n Episode {ep} (Total step = {self.total_step}): Total Reward = {score:.3f}")
                wandb.log({"episode": ep, "return": score})
                score = 0.0

            # Gradient update once we have enough samples and have finished warm-up.
            if len(self.memory) >= self.batch_size and self.total_step > self.initial_random_steps:
                losses = self.update_model()
                actor_loss = losses[0].cpu().numpy()
                qf_loss = losses[1].cpu().numpy()
                vf_loss = losses[2].cpu().numpy()
                alpha_loss = losses[3].cpu().numpy()

            wandb.log({
                "step": self.total_step,
                "actor loss": actor_loss,
                "q loss": qf_loss,
                "v loss": vf_loss,
                "alpha loss": alpha_loss,
                "alpha": float(self.log_alpha.exp().detach().cpu().item()),
            })

            # Periodic checkpoint + evaluation.
            if self.total_step % self.eval_interval == 0:
                self.save_checkpoint(self.checkpoint_dir, self.total_step)
                eval_mean, eval_std = self.evaluate(num_episodes=self.eval_episodes)
                print(
                    f"\n[Eval @ step {self.total_step}] "
                    f"Mean Return = {eval_mean:.2f} ± {eval_std:.2f} "
                    f"over {self.eval_episodes} episodes"
                )
                wandb.log({
                    "step": self.total_step,
                    "eval/mean_return": eval_mean,
                    "eval/std_return": eval_std,
                })

                # Reset the training env so the next training step starts cleanly.
                state = self._reset_env()
                score = 0.0

        self.env.close()

    def test(self):
        self.is_test = True
        state = self._reset_env()
        done = False
        score = 0.0
        while not done:
            action = self.select_action(state)
            next_state, reward, done = self.step(action)
            state = next_state
            score += reward
        print(f"[Test] score: {score:.3f}")
        self.is_test = False

    def _target_soft_update(self):
        """Polyak averaging of the value-network target."""
        tau = self.tau
        for t_param, l_param in zip(self.vf_target.parameters(), self.vf.parameters()):
            t_param.data.copy_(tau * l_param.data + (1.0 - tau) * t_param.data)


# ----------------------------------------------------------------------------
# Entry point
# ----------------------------------------------------------------------------

def seed_torch(seed):
    torch.manual_seed(seed)
    if torch.backends.cudnn.enabled:
        torch.backends.cudnn.benchmark = False
        torch.backends.cudnn.deterministic = True


def make_robosuite_env(seed: int, horizon: int = 1000):
    """Create the Panda-Door-Opening environment with shaped rewards.

    Note: starter's default horizon is 1000 (TA confirmed). The 200-baseline
    passing criterion is set under horizon=1000. We can optionally rerun
    eval with horizon=500 as a robustness check (see TA announcement 5/22).
    """
    raw_env = suite.make(
        env_name="Door",
        robots="Panda",
        has_renderer=False,
        has_offscreen_renderer=False,
        use_camera_obs=False,
        reward_shaping=True,
        horizon=horizon,
        control_freq=20,
    )
    env = GymWrapper(raw_env)
    # GymWrapper may not seed deterministically; we set the global seeds in __main__.
    return env


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--wandb-run-name", type=str, default="panda-door-sac")
    parser.add_argument("--lr", type=float, default=3e-4)
    parser.add_argument("--discount-factor", type=float, default=0.99)
    parser.add_argument("--tau", type=float, default=5e-3)
    parser.add_argument("--batch-size", type=int, default=256)
    parser.add_argument("--initial-random-steps", type=int, default=10000)
    parser.add_argument("--memory-size", type=int, default=1000000)
    parser.add_argument("--num-steps", type=int, default=1000000)
    parser.add_argument("--policy-update-freq", type=int, default=1)
    parser.add_argument("--hidden-dim", type=int, default=256)
    parser.add_argument("--seed", type=int, default=77)
    parser.add_argument("--eval-interval", type=int, default=10000,
                        help="Save a checkpoint and run evaluation every this many steps.")
    parser.add_argument("--eval-episodes", type=int, default=10,
                        help="Number of episodes used for each evaluation.")
    parser.add_argument("--checkpoint-dir", type=str, default="checkpoints_door",
                        help="Directory to save checkpoint files.")
    parser.add_argument("--horizon", type=int, default=1000,
                        help="Robosuite episode horizon. starter default = 1000.")
    args = parser.parse_args()

    # Seed everything.
    random.seed(args.seed)
    np.random.seed(args.seed)
    seed_torch(args.seed)

    # Weights & Biases logging.
    wandb.init(project="RL-HW3-SAC-Door", name=args.wandb_run_name, save_code=True)

    # Build env and agent.
    env = make_robosuite_env(args.seed, horizon=args.horizon)
    agent = SACAgent(env, args)
    agent.train()
